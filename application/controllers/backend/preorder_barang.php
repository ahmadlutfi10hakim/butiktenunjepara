<?php
class Preorder_barang extends CI_Controller{
    function __construct(){
        ...
        $this->load->model('backend/Preorder_barang_model','preorder_barang_model');
    ...
    public function index(){
        ...
        $data['bbaku'] = $this->preorder_barang_model->get_all_bbaku();
        $data['produksi'] = $this->preorder_barang_model->get_all_produksi();
        $produksi_belum_selesai = $this->preorder_barang_model->count_produksi_belum_selesai();
        ...
        $this->load->view('backend/v_preorder_barang', $data);
    }

    public function get_ajax_list()
    {
        $list = $this->preorder_barang_model->get_datatables();
        $data = array();
        $no = $_POST['start'];
        foreach ($list as $d) {
            $no++;
            ...
            $row[] = '<a class="btn icon btn-danger" href="javascript:void()" title="Minus" id="minus" value="'.$d->produksi_id.'"><i class="bi bi-file-minus"></i></a>&nbsp;&nbsp;<a class="btn icon btn-success" href="javascript:void()" title="Plus" id="plus" value="'.$d->produksi_id.'"><i class="bi bi-file-plus"></i></a>&nbsp;&nbsp;&nbsp;<a class="btn icon btn-primary btn-sm" href="javascript:void()" title="Hapus" id="del" value="'.$d->produksi_id.'"><i class="bi bi-trash"></i> Hapus</a>';
            $data[] = $row;
        }

        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->preorder_barang_model->count_all(),
            "recordsFiltered" => $this->preorder_barang_model->count_filtered(),
            "data" => $data,
        );
    ...
    function add(){
        ...
        $this->_validate();
        $get_new_id = $this->preorder_barang_model->get_new_kode_produksi();
        ...
        $datas = $this->preorder_barang_model->get_stocks($bahan_baku);
        error_reporting(0);
        $bx = $datas->row_array();
        ...
        if ($this->preorder_barang_model->update_entry_stocks($bahan_baku, $ajax_data_stocks)) {
            $data2 = array(
                'kode_produksi' => $NewID,
            ...
            $insert = $this->preorder_barang_model->insert_stock($data2);
            if($insert){
    ...
    public function delete()
    {
        if ($this->input->is_ajax_request()) {
            ...
            $post = $this->preorder_barang_model->single_entry($produksi_id);
            $jmlh = $post->jumlah;
            ...
            if ($this->preorder_barang_model->delete_entry($produksi_id, $hasil_penambahan_stock, $hasil_penambahan_saham, $bahan_baku)) {
    ...
    public function minus()
    {
        if ($this->input->is_ajax_request()) {
            ...
            } else if ($this->preorder_barang_model->minus_entry($produksi_id, $bahan_baku, $biaya_dikeluarkan, $harga_beli, $hasil_penambahan_stock, $hasil_penambahan_saham)) {
    ...
    public function plus()
    {
        if ($this->input->is_ajax_request()) {
            ...
            if ($stock < $jumlah_biasa) {
                $data = array('res' => "error", 'message' => "Change query error");
            } else if ($this->preorder_barang_model->plus_entry($produksi_id, $bahan_baku, $stokkekurangan, $biaya_dikeluarkan, $harga_beli, $hasil_penambahan_stock, $hasil_penambahan_saham)) {
                $data = array('res' => "success", 'message' => "Data berhasil dirubah");
            } else {
                $data = array('res' => "error", 'message' => "Change query error");
            }
    ...
    public function prosesproduksi()
    {
        if ($this->input->is_ajax_request()) {
            ...
            $datas = $this->preorder_barang_model->get_stocks_produksi($bahan_baku);
            error_reporting(0);
            $bx = $datas->row_array();
            $j = $bx['nama_stock'];
            $stock = $bx['stock'];
            ...
            if ($this->preorder_barang_model->plus_entry_produksi($NewID, $bahan_baku, $catatan_produksi, $id_users, $stock_jumlah_produksi, $hargabeli_total_bayar, $nilai_saham_totalbiaya, $jumlah_produksi, $totalbiaya)) {
                $nama_users = $this->session->userdata('name');
                $b = '<b>'.$nama_users.'</b> Melakukan Produksi Barang <b>'.$j.' sebanyak '.$jumlah_produksi.'</b>';
                $data2 = array('ket' => $b);
                $this->preorder_barang_model->insert_log_stock($data2);
                $data = array('res' => "success", 'message' => "Data berhasil dirubah");
            } else {
                $data = array('res' => "error", 'message' => "Change query error");
            }
    ...
    private function _validate()
    {
        ...
        $cek_bahan_baku = $this->preorder_barang_model->validasi_bahan($bahan_baku);
